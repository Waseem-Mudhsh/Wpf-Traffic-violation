...
{
    internal class Directorates : Isphelper
    {
        ...
        public DataRowCollection GetCollection(string SpStoredProcedureQuery, string name = null)
        {
            ...
            //DataRowCollection Result = new DataRowCollection();
            try
            {
                using (con)
                {
        ...
        public Response<bool> Operarion(SqlParameter[] parameters, string SpStoredProcedureQuery, string namenameSp = null)
        {
            SqlConnection con = new SqlConnection(@"server=" + Properties.Settings.Default.ServerName + " ;DataBase=" + Properties.Settings.Default.DatabaseName + " ;Integrated Security=True;Connect Timeout=15;Encrypt=False;TrustServerCertificate=False");
            ...
            try
            {
                using (con)
                {
                    ...
                    using (SqlCommand command = new SqlCommand(
                        ...
                        $"IF EXISTS (SELECT 1 FROM sys.procedures WHERE name = '{namenameSp}') " +
                        "BEGIN SELECT 'Exists' AS Result; END " +
                        "ELSE BEGIN SELECT 'Does Not Exist' AS Result; END", con))
                    {
                        var IsExsist = command.ExecuteScalar();
        ...
        public T Operations<T>(string SpStoredProcedureName, string name = null)
        {
            SqlConnection con = new SqlConnection(@"server=" + Properties.Settings.Default.ServerName + " ;DataBase=" + Properties.Settings.Default.DatabaseName + " ;Integrated Security=True;Connect Timeout=15;Encrypt=False;TrustServerCertificate=False");
